...
public static void main(String[] args) throws IOException{
   ...
   ReedSolomonDecoder decoder = new ReedSolomonDecoder(conf);
                  
  Path srcFile = new Path(args[0]);
    FileSystem fs = srcFile.getFileSystem(conf);
    Path parityFile = new Path(args[1]);
    FileSystem  parityFs = parityFile.getFileSystem(conf);
    boolean fixSource = true;
    long blockSize = Long.parseLong(args[2]);
    long errorOffset = Long.parseLong(args[3]);
    long limit = Long.parseLong(args[4]);
    boolean partial = false;
    OutputStream out = new FileOutputStream(new File((args[5])));
                  
    Context context = null;
    CRC32 crc = new CRC32();
    StripeInfo si = null;
    boolean recoverFromStripeStore = false;
    Block lostBlock = null;
   decoder.fixErasedBlockImpl(fs, srcFile, parityFs, parityFile, fixSource, blockSize, errorOffset, limit, partial, out, context, crc, si, recoverFromStripeStore, lostBlock);
  ...
  class DecodeOp implements Runnable {
    ...
    public void run() {
      try {
    	  System.out.println("in decode run:");
    	  isaDeInit(stripeSize, paritySize, matrix);
          performDecode();
          isaDeEnd();
      } finally {
    ...
    private void performDecode() {
      ...
      for(int i = 0; i < stripeSize + paritySize; i++){
          readByteBuf[i] = ByteBuffer.allocateDirect(count);
      }
      for(int i = 0; i < stripeSize + paritySize; i++){
          readByteBuf[i].position(0);
          readByteBuf[i].put(readBufs[i], startIdx, count);
      }
      
      isaDecode(readByteBuf, erasedLocations, count);
      
      for(int i = 0; i < erasedLocations.length; i++){
          int index = erasedLocations[i];
          readByteBuf[index].position(0);
          readByteBuf[index].get(writeBufs[i], startIdx, readByteBuf[index].remaining());
